...
export function serializeLeasingStock(stock: any): Record<string, unknown> {
  ...
  return {
    ...
    rental: Number(stock.rental ?? stock.purchasePrice ?? stock.value ?? 0) || 0,
    opsCosts: Number(stock.opsCosts ?? 0) || 0,
    industries: toStringArray(stock.industries),
...
export function mapStockRecordToLeasingStock(record: StockItemRecord): any {
  ...
  return {
    ...
    opsCosts: detailNumber(details, 'opsCosts', 0),
    industries: toStringArray(details.industries),
