    ...
    <>
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {lastUpdated && (
            <p className="text-xs text-stone-500 mt-2">Last updated: {formatRelativeTime(lastUpdated)}</p>
          )}
        </div>
        <NotificationCenter />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {isLoading ? (
          ...
          quickStats.map((stat, idx) => {
            ...
            return (
              <div key={idx} className={`${stat.color} rounded-xl p-6 border border-stone-200 flex items-start justify-between hover:shadow-md transition-shadow`}>
                <div>
                  <p className={`${stat.textColor} text-sm font-semibold mb-2`}>{stat.label}</p>
      ...
      <div className={isBroker ? "grid grid-cols-1 gap-6" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"}>
        {/* Monthly Sales - Donut Chart */}
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
          <div className="mb-4">
          ...
          {isLoading ? (
            ...
            <>
              <div className="flex items-center justify-center flex-1 min-h-64 mb-4">
                <svg viewBox="0 0 120 120" width="200" height="200" className="donut-chart">
                  <circle cx="60" cy="60" r="45" fill="none" stroke="#ef4444" strokeWidth="16" strokeDasharray={`${salesDash} ${280 - salesDash}`} strokeDashoffset="0" strokeLinecap="round" style={{ transition: 'stroke-dasharray 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }} />
                  <circle cx="60" cy="60" r="45" fill="none" stroke="#06b6d4" strokeWidth="16" strokeDasharray={`${leasingDash} ${280 - leasingDash}`} strokeDashoffset={`-${salesDash}`} strokeLinecap="round" style={{ transition: 'stroke-dasharray 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }} />
                  <circle cx="60" cy="60" r="45" fill="none" stroke="#10b981" strokeWidth="16" strokeDasharray={`${auctionDash} ${280 - auctionDash}`} strokeDashoffset={`-${salesDash + leasingDash}`} strokeLinecap="round" style={{ transition: 'stroke-dasharray 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }} />
                </svg>
              </div>
              <div className="bg-stone-50 rounded-xl p-5 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-full bg-red-400 shadow-sm animate-pulse"></div>
                    <span className="text-sm font-medium text-stone-700">Sales</span>
                  </div>
                  <span className="stat-value text-sm font-bold text-stone-900">
                    {salesPercent.toFixed(1)}% • {formatCurrency(monthlyRevenueByType.Sales)}
                  </span>
                </div>
                <div className="w-full bg-stone-200 rounded-full h-1.5 overflow-hidden">
                  <div className="progress-bar bg-red-400 h-1.5 rounded-full" style={{ width: `${salesPercent}%` }}></div>
                </div>
                
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-full bg-cyan-400 shadow-sm animate-pulse" style={{ animationDelay: '0.2s' }}></div>
                ...
                <div className="w-full bg-stone-200 rounded-full h-1.5 overflow-hidden">
                  <div className="progress-bar bg-cyan-400 h-1.5 rounded-full" style={{ width: `${leasingPercent}%`, animationDelay: '0.2s' }}></div>
                </div>
                
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-full bg-emerald-400 shadow-sm animate-pulse" style={{ animationDelay: '0.4s' }}></div>
                ...
                <div className="w-full bg-stone-200 rounded-full h-1.5 overflow-hidden">
                  <div className="progress-bar bg-emerald-400 h-1.5 rounded-full" style={{ width: `${auctionPercent}%`, animationDelay: '0.4s' }}></div>
                </div>
              </div>
            </>
          )}
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4 bg-stone-50 rounded-xl p-5">
          ...
          <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
            ...
            {isLoading ? (
              ...
              <div className="flex-1 min-h-64 flex flex-col">
                ...
                <div className="bg-stone-50 rounded-xl p-5 flex items-center justify-center gap-8">
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-0.5 bg-blue-500 rounded-full shadow-sm"></div>
                    <span className="text-sm font-medium text-stone-700">Sales</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-0.5 bg-emerald-500 rounded-full shadow-sm" style={{ strokeDasharray: '4,4' }}></div>
                    <span className="text-sm font-medium text-stone-700">Leasing</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-0.5 bg-amber-500 rounded-full shadow-sm" style={{ strokeDasharray: '2,6' }}></div>
                    <span className="text-sm font-medium text-stone-700">Auction</span>
                  </div>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md flex flex-col min-h-[360px]">
        ...
        {/* Deals & Performance */}
        <div className="bg-gradient-to-br from-white via-blue-50 to-indigo-50 rounded-2xl p-5 border border-indigo-200 shadow-lg">
          <h3 className="text-lg font-bold text-indigo-950 mb-4">Performance Metrics</h3>
          
          <div className="space-y-3">
            {/* Conversion Rate Card */}
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-4 shadow-md group cursor-pointer hover:shadow-lg transition-all">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-blue-200 text-xs font-semibold uppercase">Conversion Rate</p>
                  <p className="text-3xl font-bold text-white mt-1">{(metrics?.statistics.conversionRate || 0).toFixed(1)}%</p>
                </div>
            ...
            {!isBroker && (
              <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg p-4 shadow-md group cursor-pointer hover:shadow-lg transition-all">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-purple-200 text-xs font-semibold uppercase">Avg Deal Value</p>
            ...
            <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-lg p-4 shadow-md cursor-pointer hover:shadow-lg transition-all">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1">
                  {topBroker && (
                    <div className="w-12 h-12 rounded-lg bg-white/20 flex items-center justify-center text-white font-bold text-sm">
                      {topBroker.avatar}
                    </div>
                  )}
                  <div className="min-w-0">
                    <p className="text-xs text-amber-200 font-semibold uppercase">Top Performer</p>
                    <p className="text-sm font-bold text-white truncate">{topBroker?.name || "N/A"}</p>
                    <p className="text-xs text-amber-100">{topBroker?.closedDeals || 0} Closings</p>
                    <p className="text-xs text-amber-100">{formatCurrency(topBroker?.commission || 0)} Commission</p>
                  </div>
                </div>
                <GiTrophy className="text-white text-lg ml-2" />
              </div>
            </div>
          </div>
