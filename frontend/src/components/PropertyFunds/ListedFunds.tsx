...
import { listedFunds } from '@/data/propertyfunds';
import { formatRand } from '@/lib/currency';

export default function ListedFunds() {
  const [searchTerm, setSearchTerm] = useState('');
  const [expandedFundId, setExpandedFundId] = useState<string | null>(null);
  const [expandedAssetId, setExpandedAssetId] = useState<string | null>(null);

  const filteredFunds = useMemo(() => {
    return listedFunds.filter(fund =>
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-stone-900">Listed Funds</h2>
        <p className="text-sm text-stone-600 mt-1">Publicly traded real estate investment trusts and funds</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-blue-50 rounded-lg border border-blue-200 p-4">
          ...
          <p className="text-3xl font-bold text-blue-600 mt-1">{listedFunds.length}</p>
        </div>
        <div className="bg-green-50 rounded-lg border border-green-200 p-4">
          <p className="text-sm font-medium text-stone-600">Combined Assets</p>
          <p className="text-2xl font-bold text-green-600 mt-1">
            {formatRand(listedFunds.reduce((sum, f) => sum + f.totalAssets, 0))}
          </p>
        </div>
        <div className="bg-purple-50 rounded-lg border border-purple-200 p-4">
          <p className="text-sm font-medium text-stone-600">Total Properties</p>
          <p className="text-3xl font-bold text-purple-600 mt-1">
            {listedFunds.reduce((sum, f) => sum + f.assets.length, 0)}
          </p>
        </div>
        <div className="bg-orange-50 rounded-lg border border-orange-200 p-4">
          <p className="text-sm font-medium text-stone-600">Average Vacancy</p>
          <p className="text-3xl font-bold text-orange-600 mt-1">
            {(
              listedFunds.reduce((sum, f) => sum + f.vacancySchedules.reduce((vsum, v) => vsum + v.vacancyRate, 0), 0) /
              listedFunds.reduce((sum, f) => sum + f.vacancySchedules.length, 0)
            ).toFixed(1)}
            %
          </p>
        </div>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3">
                  ...
                  <span className="px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-800">
                    {formatRand(fund.unitPrice)} / unit
                  </span>
                </div>
      ...
      {filteredFunds.length === 0 && (
        <div className="text-center py-8 bg-stone-50 rounded-lg border border-stone-200">
          <p className="text-stone-500">No listed funds found</p>
        </div>
      )}
    </div>
  );
