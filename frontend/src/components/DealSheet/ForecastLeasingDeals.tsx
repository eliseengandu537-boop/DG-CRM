...
export default function ForecastLeasingDeals() {
  ...
  const leasingDeals = useMemo(() => {
    return data.forecastDeals
      .filter(deal => normalizeDealType(deal.moduleType) === 'Leasing')
      .map(deal => {
        ...
        return {
          ...
          grossComm: Number(deal.commissionAmount || 0) > 0
            ...
            : Number(deal.grossCommission || 0) > 0
            ? Number(deal.grossCommission)
            : expectedValue,
          companyComm: Number(deal.companyCommission || 0) > 0
            ? Number(deal.companyCommission)
            : calculateCommissionSplit(Number(deal.commissionAmount || deal.grossCommission || expectedValue)).companyComm,
          brokerComm: Number(deal.brokerCommission || 0) > 0
            ? Number(deal.brokerCommission)
            : calculateCommissionSplit(Number(deal.commissionAmount || deal.grossCommission || expectedValue)).brokerComm,
          coBrokerSplits: deal.coBrokerSplits,
        };
      });
  }, [data.forecastDeals, brokerById, dealById, propertyById]);

  const filteredDeals = useMemo(() => {
    return leasingDeals.filter(deal => {
  ...
  }, [leasingDeals, searchTerm, selectedQuarter]);
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold text-stone-950">Leasing Forecast</h2>

      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="p-4 bg-stone-50 rounded-lg border border-stone-200">
          <p className="text-stone-600 text-sm font-medium">Total Pipeline Value</p>
          <p className="text-3xl font-bold text-stone-900 mt-2">{formatCurrency(totalExpectedValue)}</p>
        ...
        <div className="p-4 bg-stone-50 rounded-lg border border-stone-200">
          ...
          <p className="text-3xl font-bold text-stone-900 mt-2">{formatCurrency(totalWeightedValue)}</p>
        </div>
        <div className="p-4 bg-stone-50 rounded-lg border border-stone-200">
          <p className="text-stone-600 text-sm font-medium">Gross Commission</p>
          <p className="text-3xl font-bold text-stone-900 mt-2">{formatCurrency(totalGrossComm)}</p>
        </div>
        <div className="p-4 bg-sky-50 rounded-lg border border-sky-200">
          <p className="text-stone-600 text-sm font-medium">Company (55%)</p>
          <p className="text-3xl font-bold text-sky-700 mt-2">{formatCurrency(totalCompanyComm)}</p>
        </div>
        <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
          <p className="text-stone-600 text-sm font-medium">Broker (45%)</p>
          <p className="text-3xl font-bold text-emerald-700 mt-2">{formatCurrency(totalBrokerComm)}</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-stone-200 bg-stone-50">
              ...
              <th className="px-4 py-3 text-left font-medium text-stone-700">Co-Brokers</th>
          ...
          <tbody>
            {filteredDeals.map(deal => (
              <tr key={deal.id} className="border-b border-stone-200 hover:bg-stone-50">
                ...
                <td className="px-4 py-3 text-right text-stone-900 font-medium">{formatCurrency(deal.grossComm)}</td>
                <td className="px-4 py-3 text-right text-stone-700">{deal.probability}%</td>
                <td className="px-4 py-3 text-right text-sky-700 font-medium">{formatCurrency(deal.companyComm)}</td>
                <td className="px-4 py-3 text-right text-emerald-700 font-medium">{formatCurrency(deal.brokerComm)}</td>
                <td className="px-4 py-3 text-stone-700 text-xs">
                  {Array.isArray(deal.coBrokerSplits) && deal.coBrokerSplits.length > 0
                    ? deal.coBrokerSplits.map((s: any, i: number) => (
                        <div key={i}>{s.splitPercent}% → {formatCurrency(s.brokerShare ?? 0)}</div>
                      ))
                    : '-'}
                </td>
      ...
      {!isLoading && filteredDeals.length === 0 && (
        <div className="text-center py-8 bg-stone-50 rounded-lg border border-stone-200">
          <p className="text-stone-500">No leasing forecast deals found for the selected filters</p>
        </div>
      )}
    </div>
  );
